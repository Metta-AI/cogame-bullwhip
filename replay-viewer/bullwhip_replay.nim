## Bullwhip static replay viewer, wasm side.
##
## JS hands the raw replay bytes to bw_load_replay; this module parses
## them with the SAME sim code the game server runs, re-derives the
## per-event chain states, and exposes the enriched payload (identical
...
proc bwLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "bw_load_replay", cdecl.} =
  try:
    lastError = ""
    let replay = parseJson(bytesFromPointer(data, int(length)))
    var config = defaultGameConfig()
    config.weeks = replay["config"]{"weeks"}.getInt(36)
    ...
    payload = $ %*{
      "type": "replay",
      "protocol": replay{"protocol"}.getStr("bullwhip.replay.v1"),
      "names": replay["names"],
...
proc bwPayloadPointer(): ptr uint8 {.exportc: "bw_payload_ptr", cdecl.} =
  if payload.len == 0:
    nil
  else:
    cast[ptr uint8](payload[0].addr)

proc bwPayloadLength(): cint {.exportc: "bw_payload_len", cdecl.} =
  cint(payload.len)

proc bwErrorPointer(): ptr uint8 {.exportc: "bw_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc bwErrorLength(): cint {.exportc: "bw_error_len", cdecl.} =
  cint(lastError.len)
